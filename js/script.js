...
(function () {
  ...
  /* ─────────── SIMULADOR ─────────── */
  var elValor = $('#sim-valor'), elPrazo = $('#sim-prazo');
  if (elValor && elPrazo) {
    var PLANOS = {
      auto:    { taxa: 0.17, min: 25000,  max: 400000,  step: 5000,  pMin: 24, pMax: 100, label: 'automóvel' },
      imovel:  { taxa: 0.22, min: 80000,  max: 1200000, step: 10000, pMin: 60, pMax: 220, label: 'imóvel' },
      servico: { taxa: 0.20, min: 5000,   max: 80000,   step: 1000,  pMin: 18, pMax: 72,  label: 'serviços' },
      pesado:  { taxa: 0.18, min: 90000,  max: 900000,  step: 10000, pMin: 36, pMax: 150, label: 'pesados' }
    };
    var tipo = 'auto';
    ...
    function clampRange(input, min, max, step, prefer) {
      input.min = min; input.max = max; input.step = step;
      var v = prefer != null ? prefer : +input.value;
    ...
    function render() {
      ...
      var prazo = +elPrazo.value;
      var total = valor * (1 + p.taxa);
      var parcela = total / prazo;

      $('#out-valor').textContent   = brl(valor);
      $('#out-prazo').textContent   = prazo + ' meses';
      $('#out-parcela').textContent = Math.round(parcela).toLocaleString('pt-BR');
      $('#out-total').textContent   = brl(total);
      $('#out-taxa').textContent    = (p.taxa * 100).toFixed(0) + '%';
      $('#out-extra').textContent   = brl(total - valor);

      $$('.sim-scale')[0].innerHTML = '<span>' + brl(p.min) + '</span><span>' + brl(p.max) + '</span>';
      $$('.sim-scale')[1].innerHTML = '<span>' + p.pMin + ' meses</span><span>' + p.pMax + ' meses</span>';

      var msg = 'Olá Rafael! Simulei no site: consórcio de ' + p.label +
                ', carta de ' + brl(valor) + ' em ' + prazo + ' meses (parcela aprox. ' +
                brl(parcela) + '). Pode me explicar melhor?';
      $('#sim-cta').href = 'https://wa.me/' + WPP + '?text=' + encodeURIComponent(msg);
    }

    $$('.seg-b').forEach(function (b) {
      b.addEventListener('click', function () {
        $$('.seg-b').forEach(function (x) { x.classList.remove('is-on'); });
        b.classList.add('is-on');
        tipo = b.getAttribute('data-tipo');
        var p = PLANOS[tipo];
        clampRange(elValor, p.min, p.max, p.step, null);
        clampRange(elPrazo, p.pMin, p.pMax, 2, null);
        render();
      });
    });

    elValor.addEventListener('input', render);
    elPrazo.addEventListener('input', render);

    var p0 = PLANOS.auto;
    clampRange(elValor, p0.min, p0.max, p0.step, 90000);
    clampRange(elPrazo, p0.pMin, p0.pMax, 2, 80);
    render();
  }
})();
